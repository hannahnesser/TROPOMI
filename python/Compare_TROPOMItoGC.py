...
def read_tropomi(filename):
    ...
    print('TROPOMI surface pressure: ', 
          np.nanmin(surface_pressure),
          np.nanmax(surface_pressure))
    print('TROPOMI pressure interval: ', 
          np.nanmin(pressure_interval),
          np.nanmax(pressure_interval))
...
for index in range(1):#range(0,len(Sat_files)):
	...
	#mrew
        ...
        print('------------------')
        print('TROPOMI column: ', GC_base_posteri)
        print('Lat: %.2f    Lon: %.2f' % (TROPOMI['latitude'][iSat, jSat],
                                          TROPOMI['longitude'][iSat, jSat]))
        print('GC Levels: ', GC_CH4)
        print('AK Levels:')
        for ll in range(12):
            print(GC_WEIGHT[ll]*(priori[ll]/dry_air_subcolumns[ll]*1e9+AK[ll]*(Sat_CH4[ll] - priori[ll]/dry_air_subcolumns[ll]*1e9)))
